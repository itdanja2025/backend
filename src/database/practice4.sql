...
select customer from orders group by customer; -- [v]
-- [문제 2 재현 ] books 테이블에서 장르(genre) 목록을 중복 없이 조회하는 SQL을 작성하세요.
select genre from books group by genre;
-- [문제 3 지성 ] orders 테이블에서 도서번호(book_id)별 주문이 존재하는 도서 목록을 조회하세요. (중복 제거)
select book_id from orders group by book_id;
-- [문제 4 도경 ] books 테이블에서 저자(author)별로 그룹화하여 저자 목록만 조회하세요.
select author from books group by author;
-- [문제 5 연지 ] orders 테이블에서 전체 주문 건수를 구하세요. (COUNT(*))
select count( * ) from orders;
-- [문제 6 태현 ] orders 테이블에서 전체 주문수량(order_qty)의 합계를 구하세요. (SUM())
select sum( order_qty ) from orders;
-- [문제 7 소영 ] books 테이블에서 도서 가격(price)의 평균을 구하세요. (AVG())
select avg( price ) from books;
-- [문제 8 성은 ] books 테이블에서 최고가/최저가 도서 가격을 각각 구하세요. (MAX(), MIN())
select max( price ) , min( price ) from books;
-- [문제 9 태형 ] books 테이블에서 재고(stock)가 NULL이 아닌 도서 수를 구하세요. (COUNT(stock))
select count(*) from books where stock is not null;
-- [문제 10 환빈 ] orders 테이블에서 고객별 주문 건수를 조회하세요. (customer, count)
select customer , count( customer ) from orders group by customer;
-- [문제 11 한승 ] orders 테이블에서 도서번호(book_id)별 총 주문수량 합계를 조회하세요. (book_id, sum)
select book_id , sum(order_qty) from orders group by book_id;
-- [문제 14 준현 ] orders에서 고객별 총 주문수량 합계가 5 이상인 고객만 조회하세요.
select customer , sum( order_qty) as 총주문수량 from orders group by customer having 총주문수량 >= 5 ;
-- [문제 15 민서 ] orders에서 도서번호(book_id)별 주문 건수가 3회 이상인 도서만 조회하세요.
select book_id , count( order_qty ) as 주문건수 from orders group by book_id having 주문건수 >= 3; 
-- [문제 17 병모 ] books 테이블에서 가격(price) 내림차순으로 정렬하여 조회하세요.
select * from books order by price desc;
-- [문제 18 용성 ] books 테이블에서 가격(price) 내림차순 정렬 후 상위 3권만 조회하세요. (ORDER BY + LIMIT)

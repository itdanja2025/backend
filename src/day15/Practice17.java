...
public class Practice17 {
    public static void main(String[] args) {
        ...
        list.add(  "유재석" ); list.add(  "강호동" ); list.add(  "서장훈" );
        list.add(  "신동엽" ); list.add(  "유재석" );
        ...
        // [문제3]
        int[] draw = {1,3,3,7,9,9,2,10,10,8,4,4,6,6,5}; // 배열
        Set<Integer> lottoSet = new HashSet<>(); // 집합(Set) 컬렉션 객체 생성
        for( int index = 0 ; index <= draw.length-1 ; index++ ){ // 배열 순회
            lottoSet.add( draw[ index ] ); // index번째 번호를 set 컬렉션 객체 대입 < *자동 중복제거* >
        }
        ...
        map.put( "유재석" , 95 ); // key 와 value 으로 entry한쌍 구조 추가
        map.put( "강호동" , 100 );     map.put( "신동엽" , 78 );
        map.put( "유재석" , 67 ); // key는 중복 불가능
        map.put( "서장훈" , 100 );
        System.out.println("map.get(\"유재석\")  = " + map.get("유재석") ); // key 이용한 value 호출
        System.out.println("map.size() = " + map.size()); // 모든 엔트리(key:value) 총개수 호출
        map.remove( "유재석" ); // key 이용한 엔트리 삭제
        System.out.println("map = " + map);
        // [문제6]
        System.out.println("map.containsKey(\"강호동\") = " + map.containsKey("강호동"));// - containsKey("강호동") 결과 출력
        System.out.println("map.containsValue( 65 )  = " + map.containsValue( 65 ) );// - containsValue(65) 결과 출력
        // - keySet() 반복문으로 key=value 전체 출력
        for( String key : map.keySet() ) {// 방법1]
...
class Box< T >{ // < 제네릭타입 > 이란? 클래스 작성시 미리 타입 정하지 않고 인스턴스 생성시 타입 정한다. // T : 영대문자 작성한다.
    public T content;
}

...
public class Practice4 {
    public static void main(String[] args) {
        ...
        Scanner scan = new Scanner( System.in ); // 입력 준비물
        // [1]
        System.out.print("1] 첫번째 점수 : "); int point1 = scan.nextInt();
        System.out.print("1] 두번째 점수 : "); int point2 = scan.nextInt();
        int 총점 = point1 + point2; // 총점이 90점 이상이면 true 그렇지 않으면 false
        if( 총점 >= 90 ) { System.out.println("합격"); } // true 대신에 하고 싶은 명령어;
        else{  System.out.println("불합격"); } // false 대신에 하고 싶은 명령어;
        // [2.한승]
        System.out.print("2] 첫번째 정수 : ");   int n3 = scan.nextInt();
        System.out.print("2] 두번째 정수 : ");   int n4 = scan.nextInt();
        if( n3 < n4 ){ System.out.println( n4 );} //  입력받은 두 수 비교한다.
        else{ System.out.println( n3 ) ; }
        // [3.민서] 문자비교 == vs 문자열비교 .equals
        System.out.print("아이디 : ");     String id = scan.next();    // 문자열 입력받기->저장
        System.out.print("비밀번호 : ");    String pw = scan.next();    // 문자열 입력받기->저장
        if( id.equals("admin") && pw.equals("1234") ){ // 만약에 입력받은 아이디가 admin 이고 입력받은 비밀번호가 1234 이면
            System.out.println("로그인성공");
        }else{ System.out.println("로그인실패"); }
        // [4.병모] "문자열".length() : 문자열 길이 반환 함수 , if ~ else if ~ else if
        System.out.print("비밀번호 입력하세요 : "); String pw2 = scan.next();
        if( pw2.length() >= 12 ){ System.out.println("보안등급:강함"); }
        else if( pw2.length() >= 8 ){ System.out.println("보안등급:보통"); }
        else{ System.out.println("보안등급:약함"); }
        // [5.태형] "문자열".charAt( 인덱스 ) : 문자열내 인덱스 번째 문자 1개 반환 함수, 인덱스란? 자료가 저장된 순서이며 0번시작, 문자=리터럴,문자열=배열
        System.out.print("주민등록번호 입력 : ");   String s4 = scan.next();

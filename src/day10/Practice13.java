...
public class Practice13 {
    public static void main(String[] args) {
        // [문제1] 다형성특징 : 상속 ( +객체 ) vs 인터페이스 ( -객체 )
        Cat cat = new Cat();    Dog dog = new Dog();
        Soundable soundable = cat;  soundable.makeSound(); // 야옹
        soundable = dog;            soundable.makeSound(); // 멍멍
        // [문제5]
        Flyable flyable = new Duck(); // Duck 클래스가 flyable 구현 했기때문에 가능 ( 다형성 : 타입변환 가능 )
        Swimmable swimmable = new Duck(); // Duck 클래스가Swimmable 구현 했기 떄문에 가능 ( 다형성 : 타입변환 가능 )
        flyable.fly();   swimmable.swimmable();
    }
} // CLASS END

interface Flyable{ void fly(); } // public abstract 생략가능
interface Swimmable{ void swimmable(); }
class Duck implements Flyable , Swimmable{ // 구현은 2개이상 인터페이스 가능하다.
    @Override public void fly() {System.out.println("하늘을 납니다.");}
    @Override public void swimmable() {System.out.println("물에서 헤엄칩니다.");}
}
interface Soundable{  public abstract void makeSound();  } // 추상메소드는 { } 구현부가 없다
class Cat implements Soundable{  // 해당 추상메소드를 가진 인터페이스를 구현하면 구현(객)체 라고 한다.
    @Override public void makeSound() { System.out.println("야옹"); } // 재정의
}
